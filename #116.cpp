...
public:
    void connect(TreeLinkNode *root) {
        if(!root) return;
        TreeLinkNode *curFirst = root, *curVist;
        
        while (curFirst->left != NULL) {
            curVist = curFirst;
            while (curVist != NULL) {
                curVist->left->next = curVist->right;
                if(curVist->next != NULL)
                    curVist->right->next = curVist->next->left;
                curVist = curVist->next;
            }
